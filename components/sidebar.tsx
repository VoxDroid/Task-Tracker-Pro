  ...
  return (
    <div className="flex h-full bg-[var(--color-background)]">
      ...
      >
        {/* Header */}
        <div
          className={`border-b-2 border-[var(--color-border)] flex-shrink-0 transition-all duration-300 ${isOpen ? 'p-4 flex items-center justify-between' : 'p-2 flex justify-center'}`}
        >
          {isOpen ? (
            <>
        ...
        {/* Search */}
        <div
          className={`border-b-2 border-[var(--color-border)] flex-shrink-0 transition-all duration-300 ${isOpen ? 'p-4 block' : 'p-2 hidden'}`}
        >
          <SearchBar
            onResultClick={handleSearchResult}
            placeholder="Search tasks, projects..."
            className="w-full"
            focusTrigger={focusSearchBar}
          />
        </div>
        ...
        <nav
          className={`flex-1 overflow-y-auto transition-all duration-300 ${isOpen ? 'p-4' : 'p-2'}`}
          ref={scrollRef}
              ...
              return (
                <li key={item.name} className="transition-all duration-200">
                  <Link
                    ...
                    style={
                      {
                        color: isActive ? 'var(--color-primary-foreground)' : undefined
                      } as CSSProperties
                    }
                  ...
                  >
                    ...
                    {isOpen && (
                      <span className="ml-3 font-medium truncate transition-opacity duration-200 delay-75">
                        {item.name}
                      </span>
                    )}
                  </Link>
        ...
        {/* Info Icon - Bottom */}
        <div
          className={`border-t-2 border-[var(--color-border)] flex-shrink-0 ${isOpen ? 'p-4' : 'p-2 flex justify-center'}`}
        >
          <Link
            href="/about"
      ...
      {isOpen && (
        <div
          className="fixed inset-0 bg-black bg-opacity-50 z-30 lg:hidden top-14"
