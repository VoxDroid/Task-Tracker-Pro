...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    for (const [key, value] of Object.entries(body)) {
      if (key !== 'id' && currentProject[key] !== value) {
        ...
        if (key === 'name') {
          ...
          const oldDesc = currentProject.description
            ? `"${currentProject.description.substring(0, 50)}${currentProject.description.length > 50 ? '...' : ''}"`
            : 'none'
          const newDesc = value
    ...
    const changeSummary =
      changes.length > 0
        ? `Updated project "${currentProject.name}": ${changes.join(', ')}`
        : `Updated project "${currentProject.name}" (no field changes detected)`
