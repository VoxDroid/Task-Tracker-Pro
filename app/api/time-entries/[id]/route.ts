  ...
  try {
    ...
    if (description !== undefined && !end_time) {
      ...
      logActivity(
        ...
        'time_entry',
        entryId,
        `Updated time entry description for "${entryInfo?.task_title || 'Unknown task'}": ${oldDesc} → ${newDesc}`
      )
...
export async function DELETE(
  ...
  try {
    ...
    const duration = entryInfo?.duration
      ? `${Math.floor(entryInfo.duration / 60)}m ${entryInfo.duration % 60}s`
      : 'incomplete'
    const details = [
