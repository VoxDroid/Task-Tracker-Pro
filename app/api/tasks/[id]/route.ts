...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { title, description, project_id, status, priority, assigned_to, due_date, is_favorite } =
      body
    ...
    if (description !== undefined && description !== currentTask.description) {
      ...
      const oldDesc = currentTask.description
        ? `"${currentTask.description.substring(0, 50)}${currentTask.description.length > 50 ? '...' : ''}"`
        : 'none'
    ...
    if (assigned_to !== undefined && assigned_to !== currentTask.assigned_to) {
      ...
      changes.push(
        `assigned to: ${currentTask.assigned_to || 'unassigned'} → ${assigned_to || 'unassigned'}`
      )
    }
    ...
    if (is_favorite !== undefined && (is_favorite ? 1 : 0) !== currentTask.is_favorite) {
      ...
      changes.push(
        `favorite: ${currentTask.is_favorite ? 'yes' : 'no'} → ${is_favorite ? 'yes' : 'no'}`
      )
    }
    ...
    const changeSummary =
      changes.length > 0
        ? `Updated task "${currentTask.title}": ${changes.join(', ')}`
        : `Updated task "${currentTask.title}" (no field changes detected)`
  ...
  try {
    ...
    const details = [
      ...
      taskInfo.description &&
        `Description: "${taskInfo.description.substring(0, 50)}${taskInfo.description.length > 50 ? '...' : ''}"`,
