...
export default function LogsPage() {
  ...
  const getLogStats = () => {
    ...
    const todayLogs = safeFilteredLogs.filter(
        ...
        <div className="bg-[var(--color-surface)] p-6 rounded-2xl border-2 border-[var(--color-border)] shadow-lg mb-8">
          <div className="flex flex-col lg:flex-row lg:items-center space-y-4 lg:space-y-0 lg:space-x-6">
            ...
            <div className="flex items-center space-x-4">
              ...
              {(searchQuery ||
                actionFilter !== 'all' ||
                (dateFilter !== 'all' && (dateFilter !== 'custom' || startDate || endDate))) && (
                <button
                  onClick={() => {
                    setSearchQuery('')
                    setActionFilter('all')
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                        {Object.entries(parsedDetails)
                          ...
                          .map(([key, value]) => (
                            ...
                            >
                              <div
                                className="text-xs font-medium uppercase tracking-wide opacity-70 mb-1"
                                style={{ color: 'var(--color-text)' }}
                              >
                                {key.replace(/_/g, ' ')}
                              </div>
