...
export default function TasksPage() {
  ...
  // derive filteredTasks from the fetched tasks to avoid synchronous setState inside effects
  ...
  // Reset page to first whenever filters change; schedule update asynchronously to avoid cascading renders
  useEffect(() => {
    const t = setTimeout(() => setCurrentPage(1), 0)
    return () => clearTimeout(t)
  }, [searchQuery, favoriteTaskIds, showFavoritesOnly, tasks])

  useEffect(() => {
    if (selectedTasks.length > 0 && !showSelectionBar) {
      // schedule the state update asynchronously to avoid cascading renders
      const t = setTimeout(() => setShowSelectionBar(true), 0)
      return () => clearTimeout(t)
                ...
                <div className="relative z-10 p-6 h-full flex flex-col">
                    ...
                    {task.project?.name && (
                      ...
                      >
                        ...
                        >
                          <FolderOpen
                            size={16}
                            style={{ color: priorityConfig[task.priority].color } as CSSProperties}
                          />
                        </div>
                        <span
                  ...
                  >
                    ...
                    {task.updated_at !== task.created_at && (
                      <span>Updated {getTimeSince(task.updated_at)}</span>
                    )}
                  </div>
